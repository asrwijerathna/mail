...
//$result = mysqli_query($conn, $query);
			    ...
			    while($row=mysqli_fetch_assoc($result1)){
					if($row['acc_type']=='A'){
						$query ="SELECT letter.recived_date, letter.letter_date, letter.sender, letter.sender_no, letter.title, letter.section_no, letter.subject_no, letter.type, letter.reg_no, letter.letter_no
								FROM letter
								WHERE letter_no NOT 
								IN (
								SELECT l_no
						...
						//this code should be remove when final
						$query12 ="SELECT
						  letter.recived_date,
						  letter.recived_time,
						  letter.letter_date,
						...
						FROM
						  letter
						WHERE
						  (letter.status = 'NON' OR letter.status = '' OR letter.status = 'REC') ORDER BY recived_date DESC LIMIT {$startpoint} , {$limit}";
						...
						$query ="SELECT letter.recived_date, letter.letter_date, letter.sender, letter.sender_no, letter.title, letter.section_no, letter.subject_no, letter.type, letter.reg_no, letter.letter_no
								...
								AND letter.section_no =  '$u_sec_no'
								AND letter.subject_no =  '$u_s_no'
								ORDER BY recived_date DESC 
								LIMIT {$startpoint} , {$limit}";						
						...
						FROM
						  letter						  
						WHERE
						  (letter.status = 'NON' OR
						  letter.status = '' OR
						  letter.status = 'REC') AND
...
<h3>ක්‍රියාමාර්ග සටහන්කර නොමැති ලියුම්</h3>
